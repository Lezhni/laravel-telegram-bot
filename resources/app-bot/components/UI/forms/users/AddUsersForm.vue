...
export default {
  ...
  methods: {
    async submitUser(e) {
      ...
      await this.$store.dispatch('setUsersAddUser', {form: e.target})
          .then(res => {
            console.log(res)
            if (res.status === 201) {
              this.$root.$emit('bv::hide::modal', 'AddUsersTableItem', '#btnShow')
  ...
  computed: {
    user() {
      return this.$store.getters['getUsersAddModalUser']
    }
  }
}
</script>
